...
import { getQuizzes, getMyResults } from "../services/quizService.js";
...
export default function QuizPage() {
  ...
  const [quizType, setQuizType] = useState("All Types");
  const [page, setPage] = useState(1);
  const [myStats, setMyStats] = useState({ taken: 0, completed: 0, average: "—", certificates: 0 });
    ...
    getMyResults()
      .then((results) => {
        ...
        const avg = list.length
          ? Math.round(list.reduce((sum, r) => sum + Number(r.percentage || 0), 0) / list.length)
          : 0;
        setMyStats({
          taken: list.length,
          completed: list.length,
          average: list.length ? `${avg}%` : "—",
          certificates: list.filter((r) => Number(r.percentage) >= 80).length,
        });
      })
      .catch(() => {});
  }, [user]);
  ...
  const filteredQuizzes = useMemo(() => {
    ...
    if (activeCategory !== "all") {
      ...
      const wanted = slugMap[activeCategory] || activeCategory;
      filtered = filtered.filter((quiz) => {
        const slug = String(quiz.category || "").toLowerCase().replace(/\s+/g, "");
        return slug === wanted || slug.includes(wanted) || wanted.includes(slug);
      });
    }

    if (difficulty !== "All Levels") {
      filtered = filtered.filter(
        (quiz) => String(quiz.difficulty || "").toLowerCase() === difficulty.toLowerCase()
      );
    }
    ...
    return filtered;
  }, [search, activeCategory, sortBy, quizData, difficulty, quizType]);
  ...
  const pagedQuizzes = filteredQuizzes.slice((page - 1) * perPage, page * perPage);
        ...
        <div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setQuizType("All Types");
                  setSortBy("newest");
                  setPage(1);
                }}
                className="flex h-14 w-14 items-center justify-center rounded-2xl border border-gray-200 bg-white transition-all duration-200 hover:bg-[#C4419F] hover:text-white"
                title="Reset filters"
              >
                <Filter size={20} />
              </button>
          ...
          <div className="mt-10 flex items-center justify-center gap-3">
            ...
            <button
              ...
              className="flex h-12 w-12 items-center justify-center rounded-xl border border-gray-200 bg-white"
            >
        ...
        <div className="space-y-6">
          ...
          <SidebarSection title="Your Quiz Stats">
            <div className="grid grid-cols-2 gap-4">
              <StatCard value={String(myStats.taken)} label="Quizzes Taken" />
              <StatCard value={String(myStats.completed)} label="Completed" />
              <StatCard value={myStats.average} label="Average Score" />
              <StatCard value={String(myStats.certificates)} label="Certificates" />
            </div>
          </SidebarSection>
          ...
          <SidebarSection title="Challenge Yourself">
            ...
            <button
              type="button"
              onClick={() => {
                if (quizData[0]) navigate(`/quizzes/${quizData[0].id}`);
              }}
              className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#C4419F] px-5 py-4 font-semibold text-white transition-all duration-200 hover:bg-[#C4419F]"
            >
